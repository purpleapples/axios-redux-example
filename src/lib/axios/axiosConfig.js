...
// axios 에는 통신에 필요한 기본적인 데이터를 설정하는 것이 가능하다.
...
// get 이미지 요청
axios({
    method:'get', // method 는 axios.method_name 으로도 사용가능하며  9 가지가 존재한다.    
    url:'url',
    reponseType:'stream',    
}).then(() =>{
    reponse.data.pipe(fs.createWriteStream('image.jpg'))
});
...
const axiosInstance =  axios.create(config); // config 생략 가능

//axios의 instance method 9가지
